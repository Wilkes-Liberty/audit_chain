...
def commits_in_range(base: str, head: str):
    """(sha, subject, full message) for each non-merge commit in base..head.
    ...
    sep = "\x1e"
    out = subprocess.run(
        ["git", "log", "--no-merges", f"--format=%H{sep}%s{sep}%B%x00", f"{base}..{head}"],
        capture_output=True,
        text=True,
        check=True,
    ).stdout
...
def main() -> int:
    ...
    print(f"Scanned {len(commits)} non-merge commit(s) in "
